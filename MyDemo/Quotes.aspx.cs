    ...
    public partial class Quotes : System.Web.UI.Page
    {
        ...
        private void LoadQuotesData()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    connection.ConnectionString = connectionString;
                    SqlCommand cmd = new SqlCommand("SELECT * FROM Quotes LEFT OUTER JOIN Clients ON Quotes.ClientID = Clients.ClientID ORDER BY QuoteNumber DESC", connection);
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
                    connection.Open();
                    da.Fill(ds);
                    connection.Close();
                    ...
                    if (!object.Equals(ds.Tables[0], null))
                    {
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                            {
                                string strEdit = @"<a href=""QuoteInput.aspx?lngQuoteID=" + ds.Tables[0].Rows[i]["QuoteID"] + @"&mode=edit"">";
                                string strEditEnd = @"</a>";

                                sb.Append(@"<tr>");
                                sb.Append(@"<td><a href=""QuoteInformation.aspx?lngQuoteID=" + ds.Tables[0].Rows[i]["QuoteID"] + @""" class=""row_info""></a></td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["QuoteNumber"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["Project"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["ClientName"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["Company"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["Phone"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + ds.Tables[0].Rows[i]["Email"] + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + String.Format("{0:d}", ds.Tables[0].Rows[i]["QuoteDate"]) + strEditEnd + @"</td>");
                                sb.Append(@"<td>" + strEdit + String.Format("{0:d}", ds.Tables[0].Rows[i]["ValidDate"]) + strEditEnd + @"</td>");
                                sb.Append(@"<td>&nbsp;</td>");
                                sb.Append(@"</tr>");
                            }
                        }
                        ...
                        {
                            sb.Append(@"<tr>");
                            sb.Append(@"<td colspan=""9"">Viewing this list requires certain &quot;Quotes&quot; permissions</td>");
                            sb.Append(@"</tr>");
                        }
                    }
                    QuotesDataPlaceHolder.Controls.Add(new Literal { Text = sb.ToString() });
                }
                catch (Exception ex)
                {
                    Response.Write(ex.ToString());
